...
import { addPart, updatePart, deletePart, loadSampleData } from "../../store/inventorySlice";
...
export default function Inventory() {
  ...
  useEffect(() => {
    if (inventory.length === 0) dispatch(loadSampleData());
  ...
  const handleSave = (formData) => {
    if (selectedPart?.id) {
      ...
      dispatch(addPart({ ...formData, id: Date.now() }));
    }
  ...
  return (
    <div className="bg-gray-100 min-h-screen p-6 space-y-6">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => { dispatch(deletePart(confirm)); setConfirm(null); }}
      />
    </div>
  );
}